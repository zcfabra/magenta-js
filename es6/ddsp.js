 ...
 * limitations under the License.
 * =============================================================================
 */var Oo=n(2),ko=n(20),Ro=n(42),So=n(25),Ao=n(9);const To=(e,t,n)=>{const r=Ao.context.createBuffer(1,t.length,n);return r.copyToChannel(t,0),r};function _o(e,t,n,r,o){return new Promise(a=>{const s={loudness_db:[0],f0_hz:[0]};let i=0;for(let a=t;a<n;a++)r&&a>=o?(s.loudness_db[i]=-120,s.f0_hz[i]=-1):(s.loudness_db[i]=e.loudness_db[a],s.f0_hz[i]=e.f0_hz[a]),i+=1;a(s)})}function No(e){return e*So.b}function Fo(e){return e/So.b}class jo{constructor(e,t){this.checkpointUrl=e,t&&(this.settings=t)}async initialize(){let e;Oo.registerOp("Roll",e=>{const t=Oo.split(e.inputs[0],2,2),n=Oo.concat([t[1],t[0]],2);return t.forEach(e=>e.dispose()),n}),Oo.env().set("WEBGL_PACK",!1),Oo.env().set("WEBGL_CONV_IM2COL",!1),Oo.env().set("WEBGL_DELETE_TEXTURE_THRESHOLD",104857600),this.model=await async function(e){return await Oo.loadGraphModel(e)}(this.checkpointUrl+"/model.json");try{e=await fetch(this.checkpointUrl+"/settings.json").then(e=>e.json())}finally{if(null===this.settings)throw new Error("Passing settings is required if you do not have a settings.json file.")}this.settings={...e,...this.settings},this.initialized=!0}dispose(){this.initialized&&(this.model.dispose(),this.checkpointUrl=null,this.initialized=!1)}isInitialized(){return this.initialized}async memCheck(){return await async function(){const e=window.screen.availWidth*window.screen.availHeight,t=window.devicePixelRatio,n=Math.round(e*t*600/1048576);if(!isNaN(n)&&n<50)throw new Error(`Insufficient memory! Your device has ${n} and recommended memory is 50`);try{if(await Oo.ready(),"webgl"!==Oo.getBackend())throw new Error("It looks like your browser does not support webgl.")}catch(e){throw new Error("insufficient memory - "+e)}return!0}()}async synthesize(e,t){null!==t&&(this.settings={...this.settings,...t});const{f0_hz:n,loudness_db:r,confidences:o}=e,a=Object(Ro.c)(n,r.length,this.settings.modelMaxFrameLength),s=Object(Ro.c)(o,r.length,this.settings.modelMaxFrameLength),i=await async function(e,t){const n=t.averageMaxLoudness,r=t.meanLoudness,o=t.loudnessThreshold,a=t.meanPitch;let s,i=[];if(e.loudness_db.length>0){const t=Oo.tidy(()=>{const t=Oo.tensor1d(e.loudness_db,"float32"),r=t.max(),o=Oo.sub(n,r),a=Oo.add(t,o);return t.dispose(),r.dispose(),o.dispose(),a}),a=t.greater(o),u=await Oo.booleanMaskAsync(t,a),c=await u.array(),l=Oo.tidy(()=>{const e=c>0?u.mean():t.mean(),o=Oo.sub(r,e),a=t.add(o).add(0).clipByValue(-120,n),s=a.min(),i=e.sub(s);return a.sub(s).div(i).mul(r- -120).add(-120)}),h=Oo.reshape(e.confidences,[-1,1,1]),d=Oo.pool(h,[100,1],"avg","same"),f=d.reshape([-1]),p=Oo.lessEqual(f,.7);s=Oo.tidy(()=>{const e=p.mul(-25),t=l.add(e),n=Oo.maximum(t,-120);return e.dispose(),t.dispose(),n}),i=await s.array(),a.dispose(),t.dispose(),l.dispose(),u.dispose(),d.dispose(),f.dispose(),h.dispose(),p.dispose()}let u=[];if(e.f0_hz.length>0){const t=await Object(ko.c)(e.f0_hz),n=Oo.tidy(()=>{for(let e=0,n=t.length;e<n;++e)t[e]===-1/0&&(t[e]=0);return t}),r=Oo.lessEqual(e.confidences,.7),i=s.greater(o),c=Oo.logicalOr(r,i),l=await Oo.booleanMaskAsync(n,c),h=l.mean(),d=await Oo.sub(a,h),f=await d.array(),p=Oo.tidy(()=>{let t=f/12;t=Math.round(t);return Object(Ro.b)(e.f0_hz,t)});u=await p.array(),s.dispose(),h.dispose(),l.dispose(),d.dispose(),p.dispose(),i.dispose(),c.dispose(),r.dispose()}return{f0_hz:u,loudness_db:i}}({f0_hz:a,loudness_db:r,confidences:s},this.settings),u=[],c=i.loudness_db.length,l=Fo(c);let h=!1;const d=Fo(this.settings.modelMaxFrameLength);let f;const p=1*So.c;for(let e=0;e<l;e+=d-1){const t=Math.floor(No(e)),n=No(e+d);n>c&&(h=!0);const{f0_hz:r,loudness_db:o}=await _o(i,t,n,h,c),a=Oo.tensor1d(r,"float32"),s=Oo.tensor1d(o,"float32"),l=await this.model.predict({f0_hz:a,loudness_db:s}),f=await l.data();u.push(f),l.dispose(),a.dispose(),s.dispose()}if(l<=d)f=u[0];else{const e=[];for(let t=0;t<u.length;t++){const n=u[t];e.push(n)}f=function(e,t){const n=e.reduce((e,t)=>e+t.length,0),r=new Float32Array(n);let o=0;for(let n=0;n<e.length;n++){const u=e[n],c=n<e.length-1;if(0===n&&(r.set(u,o),o+=u.length),c){const c=e[n+1],l=o-t;for(let e=l,n=0,h=u.length-t;e<o&&n<c.length;e++,n++,h++){const t=(e-l)/(o-l);r[e]=(a=u[h],s=c[n],a*(1-(i=t))+s*i)}r.set(c.slice(t),o),o+=c.slice(t).length}}var a,s,i;return r}(e,p)}const g=f.slice(0,e.originalRecordedBufferLength).map(e=>e*(this.settings.postGain||1)),m=To(0,g,So.c),b=await Object(ko.j)(m,48e3);return await async function({audioCtx:e,arrayBuffer:t,sampleRate:n}){let r,o=To(0,t,n);const a=Ao.Offline(()=>{const e=new Ao.Compressor({attack:.001,release:.001,threshold:-6}).toDestination(),t=new Ao.Reverb({wet:.3,decay:3}).connect(e),n=new Ao.Filter(8e3,"lowpass",-24).connect(t);return new Ao.Player({url:o}).connect(n).start(),t.ready},t.length/n);r=await a;const s=r.getChannelData(0);return o=null,r.dispose(),r=null,s}({audioCtx:null,arrayBuffer:b,sampleRate:48e3})}}}])}));